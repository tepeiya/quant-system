        ...
        if self._circuit_breaker:
            try:
                ...
                if client:
                    ...
                    _cb_file = "config/circuit_breaker.json"
                    if os.path.exists(_cb_file):
                        with open(_cb_file) as _f:
                            _cb_cfg = _json.load(_f)
                        _initial = _cb_cfg.get("day_start_equity") or _cb_cfg.get("last_equity") or current_equity
                    else:
                        _initial = current_equity
                    cb_result = self._circuit_breaker.check(current_equity, float(_initial))
